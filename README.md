...
ArUco 完整位姿转换、GNSS—视觉平滑接管、视觉状态估计、短时预测以及安全高度移动甲板水平跟踪。P4 代码和消息级验收已完成，下一步是实际 PX4 SITL 跟踪对比与调参。
...
| [`src/aruco_precision_landing_cpp`](src/aruco_precision_landing_cpp/README.md) | 使用船舶 GNSS 和 ArUco 完整位姿完成会合、接管、状态估计、预测位置目标与速度前馈水平跟踪。 |
...
仿真传感器节点会将其处理为 `/deck/gps/fix` 和 `/deck/gps/velocity`。控制器已完成安全高度 GNSS 会合、移动中心搜索、完整 Marker local NED 变换、GNSS—视觉接管、视觉状态估计、短时预测及位置目标与水平速度前馈跟踪。P4 主路径全程保持安全高度，不执行下降。
...
- [P4 移动目标跟踪验收记录](docs/P4_MOVING_TARGET_TRACKING_VALIDATION.md)

## 安全提示

控制节点会自动发送 Offboard 和 Arm 命令。P4 默认 `enable_auto_land=false`，当前主路径不会下降或发送 Land 命令；预测位置和水平速度前馈已经参与 `TRACK_TARGET` 控制。建议只在 SITL 中验证；实机测试前必须重新核对相机外参、PX4 坐标系、时间同步、估计器、速度与加速度限制、failsafe、通信链路和人工接管手段。
