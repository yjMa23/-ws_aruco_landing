...
2. `aruco_precision_landing_cpp`
   ...
   - 发布 PX4 Offboard 模式、轨迹设定点、飞行命令、GNSS / 视觉调试位姿、估计里程计、预测位姿和引导来源。
   - 当前 P4 主路径已实现状态：
     - `INIT`
     - `WAIT_FOR_PX4`
     - `OFFBOARD_PRE_STREAM`
...
- 静止消息级输入速度估计为零。
- NED East `0.4 m/s` 输入速度估计约为 `0.40023 m/s`。
- 全工作区累计 77 项测试通过。
...
- 使用原始视觉、估计位置或预测位置作为可配置水平目标。
- 使用甲板估计速度和相对速度阻尼生成 PX4 水平速度前馈。
- 对位置目标、前馈速度和前馈加速度进行限幅。
- 短时视觉丢失时受限预测并衰减前馈，长时丢失回退 GNSS。
...
- 静止、`0.2 m/s`、`0.4 m/s` 和正弦甲板不持续发散。
- 相比原始视觉位置模式，前馈和预测模式水平 RMSE 有明确改善。
- 设定点、速度和加速度无明显跳变。
- Marker 短时丢失时不立即失控，长时丢失能恢复到 GNSS。
- 用户确认上述真实跟踪结果后，才能进入 P5。
...
- 自动重置、启动、超时和结束单次实验；
- 每次实验 CSV/JSON、配置快照和轻量 Bag；
- 顺序批量、seed 展开、resume、失败分类和基础聚合；
- static 3 次 + constant02 3 次真实自动冒烟，6/6 PASS。

P7 自动化继续保留，不删除、不废弃。static 20 次 + constant02 20 次不再是进入 P8A 的硬门槛，统一延后到 P9 论文实验阶段。
...
P8A 重点验证相对垂直速度语义和 `TOUCHDOWN_HOLD` 对升沉甲板的相对保持。rollpitch 和 combined 继续阻断最终下降。
...
P9 复用并扩展 P7 自动化，统一执行 20+20、方法消融和论文场景。推荐比较：
