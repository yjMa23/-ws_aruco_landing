...
4. 阶段状态：
   ...
   - 当前进入 `P4` 移动甲板安全高度水平跟踪阶段。

5. 当前控制器主要缺口如下：
   - 图像采样时刻的 PX4 位姿历史插值和严格跨时间域对齐尚未实现。
   - 已估计甲板视觉位置、速度和短时预测，但尚未用于控制和速度前馈。
   - 尚未估计甲板姿态动态和角速度。
...
`P0`、`P1`、`P2.0`、`P2A`、`P2B`、`P2C`、`P2D` 和 `P3` 已完成。当前从 `P4` 开始，不直接实现 MPC 或强化学习。

下一项任务：

```text
将预测甲板水平位置和估计水平速度接入安全高度跟踪控制，
实现位置目标与速度前馈的可配置组合，并保留 P2D 原始视觉控制作为对照，
在静止、匀速和正弦甲板上统计水平误差、目标连续性和恢复行为，
暂不实现着陆窗口、下降或触地逻辑。
```

完成安全高度预测跟踪验收后，再进入 `P5：规则式着陆窗口与下降`。
