...
以上内容记录 P6B 静止甲板第一版设计。静止版控制器、状态机和默认关闭回归已经完成，后续近距视觉问题和动态平台扩展按下面的增量计划推进。

---

## 13. 2026-07-28 动态平台与分段速度增量计划

### 13.1 目标

在保留 P4.7 水平跟踪、P5C 垂直状态估计、P6A 多源触地确认及 `NAV_LAND / Disarm = 0 / 0` 边界的前提下：

- 将 P6B 扩展到纯水平运动甲板；
- 提高会合高度到近甲板阶段的下降速度；
- 只在最终接触段保持低速；
- 使用 rosbag 同时验证触地、水平跟踪和甲板确实在运动。
...
升沉场景仍需进一步区分无人机世界垂直速度与相对甲板垂直速度；倾斜和组合场景需单独验证接触稳定性。
...
```text
> 2.00 m        0.50 m/s
0.80 ~ 2.00 m   0.30 m/s
0.50 ~ 0.80 m   0.12 m/s
```

P6B 默认：

```text
0.50 → 0.25 m   0.12 m/s
0.25 → 0.15 m   0.03 m/s
```

接近阶段不能在一个控制周期跨过 `0.25 m` 减速边界。`0.15 m` 仍只是命令下限，不是触地证据。

### 13.4 自动化完成定义

- P5B 三段速率测试通过；
- P6B 接近速率、减速边界、近接触速率测试通过；
- 候选冻结、候选消失恢复、确认锁存和不安全恢复测试通过；
- PX4 报告水平运动时，低相对甲板水平速度可确认、缺失或过大的相对速度会阻断；
- 启动脚本允许水平动态场景并拒绝升沉/倾斜场景；
- 着陆窗口最低高度必须低于最终命令高度；
- 全工作区构建和测试无失败。
...
进入 `TOUCHDOWN_HOLD` 后保持至少 `10 s`，再停止并执行：

```bash
python3 scripts/evaluate_p6b_touchdown.py \
  bags/<P6B_BAG> \
  --require-moving-deck
```

只有真实 `constant02` rosbag 同时满足触地确认、水平最大误差不超过 `0.30 m`、最终阶段甲板水平位移不小于 `0.25 m`、分段速度正确、无 `NAV_LAND` 和无 Disarm，才声明动态平台首轮 P6B 通过。
