...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not intervals:
        raise RuntimeError(
            "bag never entered a P5B descent/recovery state or WAIT_LANDING_WINDOW"
        )
...
def print_human_readable(result: dict[str, Any]) -> None:
    ...
    print_statistics("P5C vertical-state deck z", result["vertical_state_deck_z"])
    ...
    print_statistics(
        "P5C vertical-state relative velocity",
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate low-altitude vertical estimation errors from a P5B ROS 2 bag."
    )
