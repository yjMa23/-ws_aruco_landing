#!/usr/bin/env python3
"""冻结 paper evaluation 输入并生成论文统计、表格、图表与可复现实验证据清单。"""
...
from experiment_utils import METRIC_FIELDS  # noqa: E402
...
PAPER_RESULTS_DOCUMENT_COMMIT = "b20c8c9186e5869242417bd6c9539f1c0d97f54f"
BOOTSTRAP_REPETITIONS = 10_000
BOOTSTRAP_SEED = 20_260_804
CONFIDENCE_LEVEL = 0.95
OUTPUT_VERSION = "paper_results_v0.1"
...
EXCLUDED_BATCHES = {
    "results/paper_baseline_20x20_20260803": "interrupted pre-freeze batch; only 4/40 completed",
    "results/paper_baseline_20x20_20260803_a9d011d": "batch orchestration contamination evidence",
    "results/paper_baseline_20x20_20260803_a9d011d_clean1": "complete pre-clock-fix baseline on superseded simulation commit",
    "results/paper_ablation_20260804_a9d011d": "SYSTEM_TIME/ROS_TIME clock-source defect evidence",
}
CLOSED_COMBINATIONS = {
...
def validate_repo_commits(repo_root: Path) -> None:
    for commit in (FORMAL_SIMULATION_COMMIT, AGGREGATION_FIX_COMMIT, PAPER_RESULTS_DOCUMENT_COMMIT):
    ...
    ancestry = subprocess.run(
        ["git", "merge-base", "--is-ancestor", PAPER_RESULTS_DOCUMENT_COMMIT, "HEAD"],
        cwd=repo_root,
        check=False,
    )
    if ancestry.returncode != 0:
        raise ValueError("HEAD does not contain the frozen paper evaluation final document commit")
...
def validate_input_paths_not_excluded(
    ...
    if contamination:
        raise ValueError(
            f"excluded historical batches were supplied as paper finalization inputs: {sorted(contamination)}"
        )
...
def validate_input_consistency(
    ...
    return {
        ...
        "paper_results_document_commit": PAPER_RESULTS_DOCUMENT_COMMIT,
...
def save_figure(figure: Any, plots_dir: Path, name: str) -> list[str]:
    ...
    for extension in ("png", "pdf", "svg"):
        ...
        if extension == "pdf":
            metadata = {"Creator": "finalize_paper_results.py", "CreationDate": None, "ModDate": None}
        elif extension == "svg":
            metadata = {"Creator": "finalize_paper_results.py", "Date": None}
...
def generate_plots(
    ...
    try:
        ...
        raise RuntimeError(f"matplotlib is required for paper plots: {error}") from error
    ...
    mpl.rcParams.update({
        ...
        "xtick.labelsize": 9,
        "ytick.labelsize": 9,
        "svg.hashsalt": "paper_evaluation-paper-results-v0.1",
    })
...
def build_markdown_summary(
    ...
    lines = [
        ...
        "## Scope",
        "",
        "This report is generated only from the frozen paper evaluation smoke, baseline, and formal-ablation structured evidence. No controller parameter, evaluator threshold, seed, or episode selection was changed, and no new SITL run was performed.",
        "",
    ...
    lines.extend([
        "",
        "Bootstrap settings: deterministic nonparametric percentile bootstrap, 10000 repetitions, fixed random seed 20260804, confidence level 0.95. Standard deviation keeps the existing paper evaluation population-standard-deviation convention.",
        "",
    ...
    lines.extend([
        ...
        "",
        "B5 roll +2 deg is the only fixed-T1 formal touchdown combination. Its result cannot be extrapolated to negative tilt, dynamic roll/pitch, rollpitch, or combined motion. terminal contact stabilization remains an Offboard-position terminal stabilization design, not PX4 attitude-setpoint control.",
        "",
...
def main() -> int:
    ...
    provenance = {
        ...
        "input_policy": "frozen explicit paper evaluation structured evidence only",
...
if __name__ == "__main__":
    try:
        ...
        print(f"paper finalization failed: {error}", file=sys.stderr)
        raise SystemExit(2)
