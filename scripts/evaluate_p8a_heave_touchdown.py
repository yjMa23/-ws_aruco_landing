...
def _load_p8a_samples(args: argparse.Namespace) -> dict[str, Any]:
    (
        rosbag2_py,
        deserialize_message,
        get_message,
    ...
    missing = REQUIRED_P8A_TOPICS - topic_types.keys()
    if missing:
        raise RuntimeError(
            "bag is missing required P8A topics: " + ", ".join(sorted(missing))
        )
    message_types = {
        topic: get_message(topic_types[topic]) for topic in REQUIRED_P8A_TOPICS
    }
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # P8A 要求世界系 z 目标随甲板运动，因此只在 P6B 子评测中关闭“目标冻结”门槛。
    base_args.maximum_hold_target_span_m = math.inf
    base_args.maximum_hold_z_velocity_mps = math.inf
    base_result = evaluate_p6b(base_args)
    samples = _load_p8a_samples(args)
    ...
    result.update(
        {
            ...
            "p8a_heave_touchdown_passed": p8a_passed,
            # 保持与现有单轮/批量实验读取接口兼容。
            "positive_touchdown_passed": p8a_passed,
        }
    )
    return result
...
def print_human_readable(result: dict[str, Any]) -> None:
    ...
    print(
        "P8A heave touchdown test: "
        + ("PASS" if result["p8a_heave_touchdown_passed"] else "FAIL")
    )


def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate P8A heave touchdown and relative-height hold."
    )
...
def main() -> int:
    ...
    return 0 if result["p8a_heave_touchdown_passed"] else 2
