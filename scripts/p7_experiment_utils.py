...
SUPPORTED_EVALUATORS = ("auto", "p4", "p5b", "p6b", "p8a", "p8c")
...
# Frozen P9 safety authorization. Absence means NOT_APPLICABLE.
APPLICABLE_COMBINATIONS = frozenset(
    {
        ...
        *(('B0', scenario, 'touchdown') for scenario in (
        ...
        # P8C-4 fixed positive T1 staged authorization.
...
class ScenarioSpec:
    """Legacy P7-compatible scenario view."""
...
class ExperimentSpec:
    """A frozen P9 method/scenario/profile combination."""
...
@dataclass(frozen=True)
class BatchConfig:
    """Legacy P7 and P9 unified batch configuration."""
...
def _parse_experiment(
    ...
    *,
    ...
    if scenario in FORBIDDEN_P9_SCENARIOS:
...
def load_batch_config(path: Path, workspace_dir: Path | None = None) -> BatchConfig:
    """Read and validate either a legacy P7 YAML or a unified P9 YAML."""
...
def make_batch_id(name: str, now: datetime | None = None) -> str:
    ...
    safe_name = safe_name.strip("-") or "p9"
