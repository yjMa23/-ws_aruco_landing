...
from evaluate_p4_bag import GeodeticPosition, world_enu_to_local_ned
from evaluate_p6b_touchdown import (
    ARM_DISARM_COMMAND,
    NAV_LAND_COMMAND,
    load_ros_modules,
    resolve_bag_uri,
...
from evaluate_p8a_heave_touchdown import (
...
P8C2_MIN_TEST_HEIGHT_HOLD_S = 8.0
P8C2_MIN_GROUND_TRUTH_SKID_CLEARANCE_M = 0.09
...
P8C1_REQUIRED_TOPICS = REQUIRED_SHADOW_TOPICS | {
    GROUND_TRUTH_TOPIC,
    STATE_TOPIC,
    TARGET_TOPIC,
    TRAJECTORY_TOPIC,
...
P8C2_REQUIRED_TOPICS = P8C1_REQUIRED_TOPICS | {
    RELATIVE_HEIGHT_TOPIC,
    HEIGHT_REFERENCE_TOPIC,
    ARUCO_VISIBLE_TOPIC,
}
P8C3_REQUIRED_TOPICS = P8C2_REQUIRED_TOPICS | {
...
def expected_attitude_deg(scenario: str) -> tuple[float, float]:
    """返回 P8C-1 场景冻结的 `(roll, pitch)` 真值，单位 degree。"""

    if scenario not in SCENARIO_ATTITUDES_DEG:
        raise ValueError(f"unsupported P8C-1 scenario: {scenario}")
...
def p8c2_clearance_threshold_definition() -> dict[str, float]:
    """返回 P8C-2 在真实实验前冻结的最小滑橇间隙推导。"""
    ...
    p8c1_worst_error_m = 0.16652911274062454
    frozen_threshold_m = P8C2_MIN_GROUND_TRUTH_SKID_CLEARANCE_M
    return {
        ...
        "p8c1_worst_absolute_skid_geometry_error_m": p8c1_worst_error_m,
        ...
        "additional_unallocated_margin_m": (
            theoretical_min_m - frozen_threshold_m - p8c1_worst_error_m
        ),
    }


def p8c2_safe_descent_gate(metrics: dict[str, Any]) -> dict[str, Any]:
    """按冻结阈值判定 P8C-2 安全下降，不把观察量冒充 PASS 门。"""

    thresholds = {
        "test_height_hold_duration_min_s": P8C2_MIN_TEST_HEIGHT_HOLD_S,
        "minimum_ground_truth_skid_clearance_m": P8C2_MIN_GROUND_TRUTH_SKID_CLEARANCE_M,
    ...
    for state in states:
        if state in P8C2_FORBIDDEN_STATES:
    ...
    return {
        ...
        "clearance_threshold_definition": p8c2_clearance_threshold_definition(),
    }


def p8c3_touchdown_gate(metrics: dict[str, Any]) -> dict[str, Any]:
    """按 P8C-3 冻结门判定固定正 2° 甲板真实触地。
...
def p8c4_counterfactual_bias_from_normal(
...
def p8c4_replay_failure_explanation(
    p8c3_metrics: Optional[dict[str, Any]],
) -> str:
    """把既有硬门结果归类为通过、滑移或姿态发散证据。"""

    if not p8c3_metrics:
        return "insufficient_p8c3_metrics"
    contact = p8c3_metrics.get("contact_metrics", p8c3_metrics)
    ...
    if p8c3_metrics.get("p8c3_touchdown_passed") is True:
        return "historical_p8c3_pass"
    return "other_frozen_gate_failure"


def p8c4_tracking_state_authorized(mode: str, state: Optional[str]) -> bool:
...
def p8c4_stabilization_gate(metrics: dict[str, Any]) -> dict[str, Any]:
    """应用真实实验前冻结的 P8C-4 功能自身门。
...
def validate_required_topics(
    ...
    """缺失任一 P8C-0 shadow 话题时抛出带话题名的 RuntimeError。"""

    missing = set(required_topics) - set(available_topics)
    if missing:
        raise RuntimeError(
            "bag is missing required P8C-0 topics: " + ", ".join(sorted(missing))
        )
...
def _load_samples(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.p8c3_touchdown:
        validate_required_topics(topic_types, P8C3_REQUIRED_TOPICS)
    elif args.p8c2_safe_descent:
        validate_required_topics(topic_types, P8C2_REQUIRED_TOPICS)
    elif args.scenario is not None:
        validate_required_topics(topic_types, P8C1_REQUIRED_TOPICS)
    ...
    if args.p8c4_stabilization:
        validate_required_topics(topic_types, P8C4_REQUIRED_TOPICS)
    ...
    replay.update(
        {
            ...
            "evidence_scope": (
                "real accepted horizontal P6B descent/touchdown Bag; used to quantify "
...
def _p8c3_touchdown_metrics(
    ...
    gate = p8c3_touchdown_gate(gate_metrics)
    return {
        ...
        "p8c3_touchdown_passed": gate["passed"],
    }


def _p8c4_historical_replay_metrics(
    args: argparse.Namespace,
    loaded: dict[str, Any],
    numeric: dict[str, list[TimedSample]],
    strings: dict[str, list[tuple[float, str]]],
    ...
    for sample in numeric[NORMAL_TOPIC]:
        ...
        if authorized:
            ...
            try:
                ...
                target_bias = p8c4_counterfactual_bias_from_normal(sample.values)
                valid = True
    ...
    return {
        ...
        "failure_explanation": p8c4_replay_failure_explanation(p8c3_metrics),
        "ground_truth_control_isolation": (
            "counterfactual command uses recorded visual normal and landing state only; "
            "Ground Truth remains limited to offline P8C-3 metric comparison"
        ),
...
def _p8c4_stabilization_metrics(
    args: argparse.Namespace,
    loaded: dict[str, Any],
    numeric: dict[str, list[TimedSample]],
    strings: dict[str, list[tuple[float, str]]],
    ...
    for sample in desired_attitude:
        ...
        if not p8c4_tracking_state_authorized(args.p8c4_mode, tracking_state):
            continue
    ...
    gate = p8c4_stabilization_gate(p8c4_gate_metrics)
    return {
        "mode": args.p8c4_mode,
        "activation_time_s": activation_time_s,
        "activation_sample_count": len(activation_times),
        ...
        "hard_pass_gate": gate,
        "p8c4_stabilization_passed": gate["passed"],
        "ground_truth_control_isolation": (
            "all P8C-4 control diagnostics are generated online from visual/PX4 data; "
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    forbidden_states = (
        []
        if args.p8c2_safe_descent or args.p8c3_touchdown
        else [state for state in states if state in FORBIDDEN_SAFE_ALTITUDE_STATES]
    )
    p8c2_forbidden_states = [state for state in states if state in P8C2_FORBIDDEN_STATES]
    ...
    evaluation_start_s = (
        ...
        if args.p8c3_touchdown
        else (descent_start_s if args.p8c2_safe_descent else tracking_start_s)
    )
    ...
    p8c2_time_sync_failures = 0
    horizontal_errors_m: list[float] = []
    if args.p8c2_safe_descent and descent_start_s is not None:
    ...
    p8c2_relative_heights = values_after(RELATIVE_HEIGHT_TOPIC, descent_start_s)
    p8c2_height_references = values_after(HEIGHT_REFERENCE_TOPIC, descent_start_s)
    p8c2_tangential_position_norms = [
        math.sqrt(sum(value * value for value in sample.values))
        for sample in numeric[TANGENTIAL_POSITION_TOPIC]
        if descent_start_s is not None and sample.time_s >= descent_start_s
    ]
    p8c2_tangential_velocity_norms = [
        math.sqrt(sum(value * value for value in sample.values))
        for sample in numeric[TANGENTIAL_VELOCITY_TOPIC]
        if descent_start_s is not None and sample.time_s >= descent_start_s
    ]
    p8c2_gt_minimum_clearances = [
        sample.values[0]
        for sample in gt_minimum_clearance_samples
        if descent_start_s is not None and sample.time_s >= descent_start_s
    ]
    p8c2_gt_skid_clearances = [
        [
            sample.values[0]
            for sample in samples
            if descent_start_s is not None and sample.time_s >= descent_start_s
    ...
    ground_truth_contact_count = sum(value <= 0.005 for value in p8c2_gt_minimum_clearances)
    ground_truth_penetration_count = sum(value < 0.0 for value in p8c2_gt_minimum_clearances)
    ...
    if args.p8c3_touchdown:
        required_message_topics |= P8C3_REQUIRED_TOPICS
    if args.p8c4_stabilization:
        required_message_topics |= {
            TERMINAL_ENABLED_TOPIC,
            TERMINAL_MODE_TOPIC,
    ...
    p8c1_passed = bool(
        ...
        and not args.p8c2_safe_descent
        and not args.p8c3_touchdown
        and data_valid
        and safety_passed
    ...
    p8c2_gate_metrics = {
        ...
        "ground_truth_minimum_skid_clearance_m": (
            min(p8c2_gt_minimum_clearances) if p8c2_gt_minimum_clearances else None
        ),
    ...
    p8c2_gate = (
        p8c2_safe_descent_gate(p8c2_gate_metrics)
        if args.p8c2_safe_descent
        else None
    )
    p8c2_passed = bool(
        args.p8c2_safe_descent
        and data_valid
        and p8c2_gate is not None
        and p8c2_gate["passed"]
        and bool(p8c2_gt_minimum_clearances)
        and bool(horizontal_errors_m)
    )
    p8c3_metrics = (
        _p8c3_touchdown_metrics(
            args,
            loaded,
            numeric,
            strings,
    ...
    p8c3_passed = bool(
        args.p8c3_touchdown
        and data_valid
        and p8c3_metrics is not None
        and p8c3_metrics["p8c3_touchdown_passed"]
    )
    p8c4_replay_metrics = (
        _p8c4_historical_replay_metrics(
            args,
            loaded,
            numeric,
            strings,
    ...
    p8c4_passed = bool(
        args.p8c4_stabilization
        and data_valid
        and p8c4_metrics is not None
        and p8c4_metrics["p8c4_stabilization_passed"]
    )
    ...
    return {
        ...
        "forbidden_p8c2_states": p8c2_forbidden_states,
        ...
        "time_sync_failure_count": time_sync_failures,
        "p8c2_horizontal_sync_failure_count": p8c2_time_sync_failures,
        ...
        "ground_truth_geometry": {
            ...
            "threshold_status": (
                "p8c2_frozen_ground_truth_minimum_clearance_gate"
                if args.p8c2_safe_descent
                else "observation_only_plan_has_no_frozen_GT_clearance_threshold"
            ),
        },
        "p8c3_touchdown_metrics": p8c3_metrics,
        "p8c4_historical_replay_metrics": p8c4_replay_metrics,
        "p8c4_terminal_stabilization_metrics": p8c4_metrics,
        "p8c2_safe_descent_metrics": {
            "evaluation_start_s": evaluation_start_s,
            "relative_height_z_m": scalar_summary(p8c2_relative_heights),
            "height_reference_m": scalar_summary(p8c2_height_references),
            "horizontal_error_m": horizontal_summary,
            ...
            "ground_truth_skid_clearances_m": [
                scalar_summary(values) for values in p8c2_gt_skid_clearances
            ],
            "ground_truth_contact_count": ground_truth_contact_count,
            "ground_truth_penetration_count": ground_truth_penetration_count,
            "first_contact_index": (
                contact_point_index(
                    [min(values) for values in p8c2_gt_skid_clearances]
                )
                if all(p8c2_gt_skid_clearances)
                else None
            ),
            "visual_freshness": {
        ...
        "evaluation_mode": (
            "p8c4_historical_replay_on_p8c3_touchdown"
            if args.p8c4_replay
            else (
            f"p8c4_{args.p8c4_mode}_on_" + (
                "p8c3_touchdown"
                if args.p8c3_touchdown
                else (
                    "p8c2_safe_descent"
            ...
            if args.p8c3_touchdown
                ...
                "p8c2_safe_descent"
                if args.p8c2_safe_descent
                else ("p8c1_strict" if args.scenario is not None else "p8c0_regression")
            )
            )
            )
        ),
        "p8c0_shadow_geometry_data_valid": data_valid,
        "safe_altitude_safety_passed": safety_passed,
        "p8c1_validation_passed": p8c1_passed,
        "p8c2_safe_descent_passed": p8c2_passed,
        "p8c3_touchdown_passed": p8c3_passed,
        "p8c4_terminal_stabilization_passed": p8c4_passed,
        "positive_touchdown_passed": (
            p8c3_passed and (p8c4_passed if args.p8c4_stabilization else True)
            if args.p8c3_touchdown
            else False
        ),
        "final_result": (
            ...
            if (
                (
                    ...
                    if args.p8c3_touchdown
                        ...
                        p8c2_passed
                        if args.p8c2_safe_descent
                        else (p8c1_passed if args.scenario is not None else data_valid)
                    )
                )
                and (p8c4_passed if args.p8c4_stabilization else True)
            )
            else "FAIL"
        ),
    }
...
def compare_evaluation_results(results: Sequence[dict[str, Any]]) -> dict[str, Any]:
    """Summarize cross-bag 0°/±2° separation and global P8C-1 safety evidence."""
...
def print_human_readable(result: dict[str, Any]) -> None:
    ...
    if result.get("p8c4_historical_replay_metrics") is not None:
        print(
            "P8C-4 historical counterfactual replay: "
            f"{result['p8c4_historical_replay_metrics']}"
        )
    if result.get("p8c4_terminal_stabilization_metrics") is not None:
        print(
            "P8C-4 terminal stabilization metrics: "
            f"{result['p8c4_terminal_stabilization_metrics']}"
        )
        print(
            "P8C-4 terminal stabilization: "
            + (
                "PASS"
                if result["p8c4_terminal_stabilization_passed"]
                else "FAIL"
            )
        )
    if "p8c3_touchdown" in str(result.get("evaluation_mode")):
        print(f"P8C-3 metrics: {result['p8c3_touchdown_metrics']}")
        print(
            "P8C-3 fixed-tilt touchdown: "
            + ("PASS" if result["p8c3_touchdown_passed"] else "FAIL")
        )
    elif "p8c2_safe_descent" in str(result.get("evaluation_mode")):
        print(f"P8C-2 metrics: {result['p8c2_safe_descent_metrics']}")
        print(
            "P8C-2 safe descent: "
            + ("PASS" if result["p8c2_safe_descent_passed"] else "FAIL")
        )
    print(
        "P8C-0 shadow geometry data: "
        + ("PASS" if result["p8c0_shadow_geometry_data_valid"] else "FAIL")
    )
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate P8C-0/P8C-1 shadow geometry, P8C-2 fixed-tilt safe "
            "descent, P8C-3 fixed-tilt touchdown, and P8C-4 terminal "
            "stabilization diagnostics from a rosbag."
        )
    )
    ...
    parser.add_argument(
        ...
        help="recorded P8C scenario; required for strict P8C-1/P8C-2/P8C-3 evaluation",
    )
    parser.add_argument(
        "--p8c2-safe-descent",
        action="store_true",
        help=(
            "apply the frozen P8C-2 DESCEND -> TEST_HEIGHT_HOLD, non-contact, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "apply the frozen P8C-3 FINAL_DESCENT -> TOUCHDOWN_CANDIDATE_HOLD -> "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "counterfactually replay frozen P8C-4 B1 command on a historical P8C-3 bag; "
    ...
    parser.add_argument(
        "--p8c4-stabilization",
        action="store_true",
        help="apply the frozen P8C-4 terminal command, tracking, fallback, and safety gates",
    )
    parser.add_argument(
        "--p8c4-mode",
        choices=("shadow", "rehearsal", "active"),
        default="shadow",
        help="P8C-4 experiment mode represented by the bag",
    )
...
def main() -> int:
    ...
    try:
        ...
        if args.p8c2_safe_descent and args.p8c3_touchdown:
            raise ValueError("--p8c2-safe-descent and --p8c3-touchdown are mutually exclusive")
        if args.p8c4_replay and args.p8c4_stabilization:
            raise ValueError("--p8c4-replay and --p8c4-stabilization are mutually exclusive")
        if args.p8c4_replay:
            if not args.p8c3_touchdown:
                raise ValueError("--p8c4-replay requires --p8c3-touchdown")
            if args.scenario not in {
                "tilt_roll_pos_2deg",
                "tilt_pitch_pos_2deg",
            }:
                raise ValueError(
                    "--p8c4-replay is restricted to positive fixed +2 degree tilt scenarios"
                )
        if args.p8c4_stabilization:
            if args.scenario not in {
                "tilt_roll_pos_2deg",
                ...
                raise ValueError(
                    "--p8c4-stabilization is restricted to positive fixed +2 degree tilt scenarios"
                )
            if args.p8c4_mode == "active" and not args.p8c3_touchdown:
                raise ValueError("P8C-4 active mode requires --p8c3-touchdown")
            if args.p8c4_mode == "rehearsal" and not args.p8c2_safe_descent:
                raise ValueError("P8C-4 rehearsal mode requires --p8c2-safe-descent")
            if args.p8c4_mode == "shadow" and args.p8c3_touchdown:
                raise ValueError("P8C-4 shadow mode cannot be combined with touchdown")
        if args.p8c3_touchdown:
            if args.scenario not in {
                "static",
                "constant02",
                ...
                raise ValueError(
                    "--p8c3-touchdown requires static, constant02, "
            ...
            if args.scenario not in {
                ...
                raise ValueError(
                    "--p8c2-safe-descent requires static, constant02, "
                    "tilt_roll_pos_2deg, or tilt_pitch_pos_2deg"
                )
            args.require_ground_truth = True
        elif args.scenario is not None:
        ...
        if args.marker_history_bag is not None:
            ...
            if not marker_history["passed"]:
                result["p8c1_validation_passed"] = False
    ...
    if args.json:
        ...
        print_human_readable(result)
    passed = (
        result["p8c3_touchdown_passed"]
        if args.p8c3_touchdown
        else (
            result["p8c2_safe_descent_passed"]
            if args.p8c2_safe_descent
            else (
                result["p8c1_validation_passed"]
                if args.scenario is not None
                else result["p8c0_shadow_geometry_data_valid"]
            )
        )
    )
