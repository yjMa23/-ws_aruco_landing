#!/usr/bin/env python3
"""离线评测 relative descent 相对甲板高度分阶段下降 rosbag。"""
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not descent_times:
        raise RuntimeError("bag never entered a relative descent state")
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate relative-height descent metrics from a ROS 2 bag."
    )
