#!/usr/bin/env python3
"""离线评测 horizontal tracking 移动甲板水平跟踪 rosbag。
...
def mpc_status_is_intentional_disengagement(status: str) -> bool:
    return status.upper() == "TERMINAL_RULE_BASED_TRACKING"
...
def evaluate(args: argparse.Namespace) -> dict[str, float | int | str]:
    ...
    if stable_statuses:
        result["mpc_terminal_rule_based_tracking_count"] = sum(
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate horizontal tracking moving-deck tracking metrics from a ROS 2 bag."
    )
