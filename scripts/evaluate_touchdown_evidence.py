#!/usr/bin/env python3
"""离线评测 touchdown evidence 多源触地候选与确认调试话题。"""
...
ALLOWED_TOUCHDOWN_EVIDENCE_STATE = "TEST_HEIGHT_HOLD"
...
def evaluate(path: Path) -> dict[str, Any]:
    ...
    while reader.has_next():
        ...
        if topic == STATE_TOPIC:
            ...
            if status == "CANDIDATE":
                candidate_samples += 1
                if current_state != ALLOWED_TOUCHDOWN_EVIDENCE_STATE:
                    candidate_outside_allowed_state += 1
            elif status == "CONFIRMED" and current_state != ALLOWED_TOUCHDOWN_EVIDENCE_STATE:
...
def print_human_readable(result: dict[str, Any]) -> None:
    ...
    print(
        "negative touchdown-evidence test: "
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate touchdown evidence and false confirmations."
    )
