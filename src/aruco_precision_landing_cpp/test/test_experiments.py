...
import evaluate_horizontal_tracking  # noqa: E402
import evaluate_final_descent_touchdown  # noqa: E402
...
class ExperimentTests(unittest.TestCase):
    ...
    def test_heave_touchdown_heave_profile_is_supported_by_batch_automation(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            config_path = root / "heave_touchdown.yaml"
            config_path.write_text(
                textwrap.dedent(
                    """
                    name: heave_touchdown-h1
                    output_root: results
    ...
    def test_tilted_deck_touchdown_positive_fixed_tilt_profiles_are_supported_by_automation(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            config_path = root / "tilted_deck_touchdown.yaml"
            config_path.write_text(
                textwrap.dedent(
                    """
                    name: tilted_deck_touchdown
                    output_root: results
                    scenarios:
    ...
    def test_mpc_contact_disengagement_is_not_counted_as_solver_failure(self) -> None:
        self.assertTrue(
            evaluate_horizontal_tracking.mpc_status_is_intentional_disengagement(
                "TERMINAL_RULE_BASED_TRACKING"
            )
        )
        self.assertFalse(
            evaluate_horizontal_tracking.mpc_status_is_solver_success("TERMINAL_RULE_BASED_TRACKING")
        )
        self.assertTrue(evaluate_horizontal_tracking.mpc_status_is_solver_success("solved"))
        self.assertFalse(
            evaluate_horizontal_tracking.mpc_status_is_intentional_disengagement(
                "MAXIMUM_ITERATIONS"
            )
    ...
    def test_evaluator_selection_uses_heave_touchdown_for_heave_profiles(self) -> None:
        self.assertEqual(
            run_single_experiment.evaluator_path_for_scenario(
                WORKSPACE_DIR, "static"
            ).name,
            "evaluate_final_descent_touchdown.py",
        )
        self.assertEqual(
            run_single_experiment.evaluator_path_for_scenario(
                WORKSPACE_DIR, "heave_h1"
            ).name,
            "evaluate_heave_touchdown.py",
        )
    ...
    def test_terminal_stabilization_profile_completion_requirements_are_state_driven(self) -> None:
    ...
    def test_terminal_stabilization_staged_commands_are_strict_and_mutually_separated(self) -> None:
        common = (
            WORKSPACE_DIR,
            "tilt_roll_pos_2deg",
            1,
            Path("/tmp/terminal_stabilization-bag"),
            "close-range",
            False,
    ...
    def test_legacy_final_descent_contact_gate_uses_confirmed_time_not_preconfirm_minimum(self) -> None:
        result = evaluate_final_descent_touchdown.contact_clearance_gate(
        ...
        self.assertFalse(
            evaluate_final_descent_touchdown.contact_clearance_gate(
    ...
    def test_non_touchdown_evaluator_accepts_tilted_deck_final_result(self) -> None:
        self.assertTrue(
            run_single_experiment.evaluators_passed(
                {"final_result": "PASS"},
                None,
    ...
    def test_tilted_deck_touchdown_evaluator_selection_and_frozen_command(self) -> None:
        evaluator = run_single_experiment.evaluator_path_for_scenario(
            WORKSPACE_DIR,
            "tilt_roll_pos_2deg",
            tilted_deck_touchdown=True,
        )
        self.assertEqual(evaluator.name, "evaluate_tilted_deck.py")
        command = run_single_experiment.build_start_command(
            WORKSPACE_DIR,
            "tilt_roll_pos_2deg",
            1,
            Path("/tmp/tilted_deck_touchdown-bag"),
            "close-range",
            False,
            "PREDICTED_POSITION_VELOCITY_FF",
        )
    ...
    def test_tilted_deck_touchdown_tilt_dry_run_is_noninteractive_and_strict(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            args = SimpleNamespace(
                scenario="tilt_roll_pos_2deg",
                seed=1,
                ...
                batch_id="tilted_deck_touchdown-dry",
                episode_id="tilted_deck_touchdown-dry-roll",
            ...
            self.assertTrue(result["tilted_deck_touchdown"])
    ...
    def test_heave_dry_run_uses_graded_scenario(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            args = SimpleNamespace(
                ...
                touchdown_hold=10.0,
                output_directory=Path(directory),
                batch_id="heave_touchdown-dry",
                episode_id="heave_touchdown-dry-heave-h1",
                camera_model="close-range",
                record_camera_debug=False,
                dry_run=True,
                workspace_dir=WORKSPACE_DIR,
    ...
    def test_mpc_dry_run_passes_explicit_tracking_mode(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            args = SimpleNamespace(
                ...
                touchdown_hold=10.0,
                output_directory=Path(directory),
                batch_id="relative_mpc-dry",
                episode_id="relative_mpc-dry-heave-h1",
                camera_model="close-range",
    ...
    def test_paper_evaluation_method_config_and_episode_id(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            config_path = root / "paper_evaluation.yaml"
            config_path.write_text(
                textwrap.dedent(
                    """
                    name: paper_evaluation-test
                    output_root: results
                    experiments:
                      - method: B1
                        scenario: constant02
            ...
            plan = run_batch_experiments.expanded_plan(config, "paper_evaluation-fixed")
            self.assertIn("B1_constant02_safe_altitude", plan[0]["episode_id"])
            self.assertEqual(plan[0]["method"], "B1")

    def test_paper_evaluation_duplicate_combination_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_paper_evaluation_incompatible_matrix_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_paper_evaluation_negative_and_dynamic_tilt_remain_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_paper_evaluation_b3_b4_and_b5_frozen_parameters(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_paper_evaluation_method_identity_cannot_be_redefined(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_paper_evaluation_not_applicable_is_not_executed_or_failed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_paper_evaluation_evaluator_auto_routing(self) -> None:
        self.assertEqual(
            run_single_experiment.evaluator_path_for_scenario(
                WORKSPACE_DIR,
                "sinusoidal",
            ...
            "evaluate_horizontal_tracking.py",
        )
        self.assertEqual(
            run_single_experiment.evaluator_path_for_scenario(
                WORKSPACE_DIR,
            ...
            "evaluate_relative_descent.py",
        )
        self.assertEqual(
            run_single_experiment.evaluator_path_for_scenario(
                WORKSPACE_DIR,
                "tilt_roll_pos_2deg",
                use_tilted_deck_evaluator=True,
                experiment_profile="touchdown",
            ).name,
            "evaluate_tilted_deck.py",
        )

    def test_horizontal_tracking_safe_altitude_evaluator_does_not_receive_final_descent_flag(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            self.assertIn("evaluate_horizontal_tracking.py", command[1])
            self.assertNotIn("--require-moving-deck", command)

    def test_paper_evaluation_start_command_includes_method_parameters(self) -> None:
        command = run_single_experiment.build_start_command(
            WORKSPACE_DIR,
            "constant02",
            1,
            Path("/tmp/paper_evaluation-bag"),
            "close-range",
            False,
            "PREDICTED_POSITION_VELOCITY_FF",
            "safe-altitude",
    ...
    def test_paper_evaluation_resume_fingerprint_detects_code_change(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            episode_dir = Path(directory)
    ...
    def test_paper_evaluation_safe_altitude_primary_gate(self) -> None:
        self.assertTrue(
            run_single_experiment.evaluators_passed(
                {
    ...
    def test_paper_evaluation_aggregate_groups_and_nonfinite_values(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            batch_dir = Path(directory)
            ...
            self.assertTrue((batch_dir / "RESULTS_SUMMARY.md").is_file())
    ...
    def test_metric_value_reads_tilted_deck_nested_metrics(self) -> None:
        evaluation = {
