#!/usr/bin/env python3
"""不启动 PX4/Gazebo 的 P8C-0 evaluator、参数和 shadow 隔离测试。"""
...
from evaluate_p8c_tilted_deck import (  # noqa: E402
    ...
    p8c2_clearance_threshold_definition,
    p8c2_safe_descent_gate,
    p8c3_touchdown_gate,
    p8c4_counterfactual_bias_from_normal,
    p8c4_replay_failure_explanation,
    p8c4_stabilization_gate,
    p8c4_tracking_state_authorized,
    scalar_summary,
...
class P8CTiltedDeckTests(unittest.TestCase):
    ...
    def test_positive_fixed_tilt_preserves_p8c1_and_p8c2_modes(self) -> None:
        for scenario in ("tilt_roll_pos_2deg", "tilt_pitch_pos_2deg"):
            ...
            self.assertIn("P8C-1 fixed-tilt safe altitude", safe_altitude.stdout)
            ...
            self.assertIn("P8C-2 positive fixed-tilt safe descent", safe_descent.stdout)

    def test_positive_fixed_tilt_allows_p8c3_touchdown_only_with_frozen_flags(self) -> None:
        for scenario in ("tilt_roll_pos_2deg", "tilt_pitch_pos_2deg"):
            ...
            self.assertIn("P8C-3 positive fixed-tilt touchdown", completed.stdout)
    ...
    def test_dynamic_tilt_profiles_keep_final_descent_closed(self) -> None:
        for scenario in ("rollpitch", "combined"):
            ...
            self.assertNotEqual(completed.returncode, 0)
            self.assertIn("final-descent", completed.stdout)

    def test_p8c4_shadow_gate_accepts_only_positive_fixed_tilt(self) -> None:
        for scenario in ("tilt_roll_pos_2deg", "tilt_pitch_pos_2deg"):
            completed = self._run_start_gate(
    ...
    def test_p8c4_rehearsal_gate_requires_safe_descent_without_final_descent(self) -> None:
        for scenario in ("tilt_roll_pos_2deg", "tilt_pitch_pos_2deg"):
    ...
    def test_p8c4_active_gate_requires_full_positive_touchdown_whitelist(self) -> None:
        for scenario in ("tilt_roll_pos_2deg", "tilt_pitch_pos_2deg"):
            accepted = self._run_start_gate(
                "--scenario",
                scenario,
    ...
    def test_p8c4_defaults_and_ros_interface_are_frozen(self) -> None:
    ...
    def test_p8c4_counterfactual_replay_uses_frozen_signs_and_limits(self) -> None:
        roll_bias = p8c4_counterfactual_bias_from_normal(
        ...
        pitch_bias = p8c4_counterfactual_bias_from_normal(
            (-math.sin(math.radians(2.0)), 0.0, -math.cos(math.radians(2.0)))
        )
        self.assertLess(pitch_bias[0], 0.0)
        limited = p8c4_counterfactual_bias_from_normal(
            (math.sin(math.radians(20.0)), 0.0, -math.cos(math.radians(20.0)))
        )
        self.assertLessEqual(math.hypot(*limited), 0.45)
        with self.assertRaises(ValueError):
            p8c4_counterfactual_bias_from_normal((0.0, 0.0, 1.0))

    def test_p8c4_replay_failure_explanation_preserves_historical_failure(self) -> None:
        self.assertEqual(
            p8c4_replay_failure_explanation(
                {
    ...
    def test_p8c4_active_tracking_uses_contact_stable_states_only(self) -> None:
        self.assertFalse(p8c4_tracking_state_authorized("active", "FINAL_DESCENT"))
        self.assertTrue(
            p8c4_tracking_state_authorized("active", "TOUCHDOWN_CANDIDATE_HOLD")
        )
        self.assertTrue(p8c4_tracking_state_authorized("active", "TOUCHDOWN_HOLD"))
        self.assertTrue(p8c4_tracking_state_authorized("rehearsal", "TEST_HEIGHT_HOLD"))
        self.assertFalse(p8c4_tracking_state_authorized("rehearsal", "FINAL_DESCENT"))

    def test_p8c4_stabilization_gate_covers_frozen_feature_limits(self) -> None:
        valid = {
            "mode": "active",
        ...
        passed = p8c4_stabilization_gate(valid)
        ...
        for expected_check, override in failures.items():
            ...
            result = p8c4_stabilization_gate(metrics)
    ...
    def test_p8c3_touchdown_gate_covers_all_frozen_hard_failures(self) -> None:
        valid = {
            "state_sequence": [
                "WAIT_LANDING_WINDOW",
                "DESCEND",
        ...
        passed = p8c3_touchdown_gate(valid)
        self.assertTrue(passed["passed"], passed)
        ...
        for expected_check, override in failures.items():
            metrics = dict(valid)
            metrics.update(override)
            result = p8c3_touchdown_gate(metrics)
            self.assertFalse(result["passed"], expected_check)
            self.assertIn(expected_check, result["failed_checks"])
    ...
    def test_p8c2_clearance_threshold_is_frozen_before_real_experiments(self) -> None:
        definition = p8c2_clearance_threshold_definition()
        ...
        self.assertAlmostEqual(definition["p8c1_worst_absolute_skid_geometry_error_m"], 0.16652911274062454)
    ...
    def test_p8c2_safe_descent_gate_covers_all_hard_failures(self) -> None:
        valid = {
            "state_sequence": [
                "WAIT_LANDING_WINDOW",
                "DESCEND",
        ...
        self.assertTrue(p8c2_safe_descent_gate(valid)["passed"])
        ...
        for expected_check, override in failures.items():
            metrics = dict(valid)
            metrics.update(override)
            result = p8c2_safe_descent_gate(metrics)
            self.assertFalse(result["passed"], expected_check)
            self.assertIn(expected_check, result["failed_checks"])

    def test_calibration_gate_uses_frozen_p8c1_thresholds(self) -> None:
    ...
    def test_evaluator_treats_time_sync_failure_as_hard_failure(self) -> None:
        evaluator_source = (WORKSPACE_DIR / "scripts" / "evaluate_p8c_tilted_deck.py").read_text(
            encoding="utf-8"
        )
