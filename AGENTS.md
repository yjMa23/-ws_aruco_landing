...
* `P9` 为当前下一工程阶段：统一批量评测、方法消融和论文实验；P7 的 static/constant02 20+20 配置在该阶段执行。
...
22. `P9`：统一批量评测、消融和论文实验，当前执行阶段。

P8A、P8B 与 P8C fixed T1 已通过真实验收。P8C 当前状态为 `P8C-4 VALIDATION PASS / P8C T1 VALIDATION PASS / P8C-3 DESIGN GATE CLOSED`，验收见 `docs/validation/P8C_TILTED_DECK_LANDING_VALIDATION.md`。该结论只能用于固定正 `+2° roll/pitch` T1，不能外推到负倾角、动态 roll/pitch 或 combined；这些能力必须另立后续阶段。当前下一步是 P9 统一评测，而不是继续扩大 P8C 安全边界。强化学习不属于本轮传统高级基线实现范围。
...
下一步执行 P9 统一批量评测、消融和论文实验，并运行 P7 保留的 static/constant02 20+20；
